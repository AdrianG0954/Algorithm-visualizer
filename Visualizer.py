...
def draw(draw_info, algo_name, ascending=True):
	draw_info.window.fill(draw_info.BACKGROUND_COLOR)

	title = draw_info.LARGE_FONT.render(f"{'Algorith Visualizer'}", 1, draw_info.GREEN)
	draw_info.window.blit(title, (draw_info.width/2 - title.get_width()/2 , 5))

	sub_title = draw_info.FONT.render(f"Current algorithm: {algo_name}", 1, draw_info.RED)
	draw_info.window.blit(sub_title, (draw_info.width/2 - sub_title.get_width()/2 , 50))

	controls = draw_info.FONT.render("R - Reset | SPACE - Start Sorting | A - Ascending | D - Descending", 1, draw_info.WHITE)
	draw_info.window.blit(controls, (draw_info.width/2 - controls.get_width()/2 , 80))

	sorting = draw_info.FONT.render("I - Insertion Sort | B - Bubble Sort | Q - Quick Sort | M - Merge Sort | S - Selection Sort ", 1, draw_info.WHITE)
